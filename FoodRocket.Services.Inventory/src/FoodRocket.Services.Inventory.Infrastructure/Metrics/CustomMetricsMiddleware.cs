...
namespace FoodRocket.Services.Inventory.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/inventory/products")] = Query(nameof(GetProducts)),
            [GetKey("POST", "/inventory/products")] = Command(nameof(AddProduct))
        };
