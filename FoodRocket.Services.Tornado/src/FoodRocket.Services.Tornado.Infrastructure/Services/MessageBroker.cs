...
using OpenTracing;
namespace FoodRocket.Services.Tornado.Infrastructure.Services
{
    internal sealed class MessageBroker : IMessageBroker
    {
        ...
        public async Task PublishAsync(IEnumerable<IEvent> events)
        {
            ...
            foreach (var @event in events)
            {
                ...
                if (_outbox.Enabled)
                {
                    var eventObjectType = @event.GetType();
                    var eventInstance = _mapper.Map(@event, typeof(IEvent), eventObjectType);
                    await _outbox.SendAsync(eventInstance, originatedMessageId, messageId, correlationId, spanContext,
